...
test('页面加载成功并显示标题', async ({ page }) => {
  ...
  // 检查主标题
  await expect(page.getByText('JSON 查看工具')).toBeVisible();
});
...
test('检查所有视图按钮存在且可见', async ({ page }) => {
  ...
  // 检查编辑器视图按钮
  await expect(page.getByText('编辑器视图')).toBeVisible();
});
...
test('检查上传按钮存在且可见', async ({ page }) => {
  ...
  // 检查上传按钮
  await expect(page.getByText('上传JSON文件', { exact: true })).toBeVisible();
...
test('检查空状态显示', async ({ page }) => {
  ...
  // 检查空状态提示
  await expect(page.getByText('请上传JSON文件或在编辑器中输入JSON数据')).toBeVisible();
});